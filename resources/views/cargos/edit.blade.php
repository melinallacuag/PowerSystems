        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('cargos.update', ['cargos' => $cargos->id]) }}">
                        @csrf
                        @method('PUT')
                        <!-- Alertas de Cargos -->
                        ...
                        <div class="flex flex-wrap -mx-3 mb-12">
                            <div class="w-full md:w-1/2 px-3 mb-4 md:mb-0">
                                <x-input-label for="name" :value="__('Actualizar Cargos: ')" class="font-semibold text-green-600" />
                            </div>
                        </div>
                        <!-- Nombre de la Categoria -->
                        <div class="flex flex-wrap -mx-3 mb-12">
                            <div class="w-full md:w-1/2 px-3 mb-4 md:mb-0">
                                <x-input-label for="name" :value="__('Nombre del Cargo  *')" />
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="$cargos->name" autofocus/>
                            </div>
                        </div>
     ...
     document.addEventListener('DOMContentLoaded', function () {
        ...
        btnRegister.forEach(btn => {
            btn.addEventListener('click', function (e) {
                ...
                if(name === ''){
                    isValid = false;
                    errorMessages.push('* El campo nombre del cargo es obligatorio.');
                }
