<nav x-data="{ open: false, showAdmin: false }" class="bg-white border-b border-green-100">
                ...
                @if (Auth::user()->rol == 'admin')
                    ...
                    <div class="hidden sm:flex sm:items-center sm:ms-6">
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                <x-dropdown-link :href="route('categoria.index')" :active="request()->routeIs('categoria.index')">
                                    {{ __('Categoría') }}
                                </x-dropdown-link>

                                <x-dropdown-link :href="route('cargos.index')" :active="request()->routeIs('cargos.index')">
                                    {{ __('Cargos') }}
                                </x-dropdown-link>

                                <x-dropdown-link :href="route('service.index')" :active="request()->routeIs('service.index')">
                                    {{ __('Servicios') }}
                                </x-dropdown-link>
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        ...
        @if (Auth::user()->rol == 'admin')
            ...
            <!-- Responsive Administración Dropdown -->
            <div class="pt-2 pb-3 space-y-1">
                <button @click="showAdmin = !showAdmin"
                    class="flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                    {{ __('Recursos') }}
                    <svg class="fill-current h-4 w-4 ml-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                        <path :class="{ 'hidden': showAdmin, 'inline-flex': !showAdmin }" class="inline-flex"
                            stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 6h16M4 12h16M4 18h16" />
                        <path :class="{ 'hidden': !showAdmin, 'inline-flex': showAdmin }" class="hidden"
                            stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            </div>

            <div x-show="showAdmin" class="space-y-1">
